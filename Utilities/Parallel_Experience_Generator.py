...
class Parallel_Experience_Generator(object):
    ...
    def __init__(self, return_env_entered, model, device):
    ...
    def reset_game(self):
        ...
        env = self.env()
        policy = copy.deepcopy(self.model) # Model(env.get_state_size(), env.get_action_size(), 2, config.hyperparameters["Policy_Gradient_Agents"]) #policy copy.deepcopy(self.policy)
...
# states_for_all_episodes, actions_for_all_episodes, rewards_for_all_episodes = obj.play_n_episodes(3)
#
