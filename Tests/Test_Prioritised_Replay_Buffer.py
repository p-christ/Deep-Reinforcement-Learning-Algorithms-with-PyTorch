...
def test_prioritised_replay_buffer():
    ...
    assert buffer.deque[0].key == 100.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.deque[0].value == (1, 2, 3, 4, 5)
    assert buffer.deque[0].heap_index == 1
    assert buffer.heap[1].key == 100.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    ...
    assert buffer.deque[1].key == 99.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.deque[1].value == (1, 2, 3, 4, 5)
    assert buffer.deque[1].heap_index == 2
    assert buffer.heap[2].key == 99.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.heap[2].value == (1, 2, 3, 4, 5)

    assert buffer.deque[2].key == 98.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.deque[2].value == (1, 2, 3, 4, 5)
    assert buffer.deque[2].heap_index == 3
    assert buffer.heap[3].key == 98.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.heap[3].value == (1, 2, 3, 4, 5)

    buffer.add_experience(105, 1, 2, 3, 4, 5)

    assert buffer.deque[3].key == 105.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.deque[3].value == (1, 2, 3, 4, 5)
    assert buffer.deque[3].heap_index == 1
    assert buffer.heap[1].key == 105.0**hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.heap[1].value == (1, 2, 3, 4, 5)
    assert buffer.heap[2].key == 100.0 ** hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]

    buffer.add_experience(101, 1, 24, 3, 4, 5)

    assert buffer.deque[0].key == 101.0 ** hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.deque[0].value == (1, 24, 3, 4, 5)
    assert buffer.deque[0].heap_index == 2
    assert buffer.heap[2].key == 101.0 ** hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
    assert buffer.heap[2].value == (1, 24, 3, 4, 5)


def test_heap_always_keeps_max_element_at_top():
    hyperparameters["DQN_Agents"]["buffer_size"] = 200
    for _ in range(100):
        buffer = Prioritised_Replay_Buffer(hyperparameters)
        elements_added = []
        ...
        max_key = np.max(elements_added)** hyperparameters["DQN_Agents"]["alpha_prioritised_replay"]
...
def test_give_sum_of_elements_is_always_correct():
    ...
    for _ in range(100):
        ...
        for ix in range(1, 100):
            element = random.random()
            elements_added.append((abs(element) + hyperparameters["DQN_Agents"]["incremental_td_error"]) ** hyperparameters["DQN_Agents"]["alpha_prioritised_replay"])
            buffer.add_experience(element, 0, 0, 0, 0, 0)
