...
class A2C(Base_Agent):
    ...
    #
    #     self.object_copies = [copy.deepcopy(self) for _ in range(processors)]
    #     self.queue = Queue()
    #
    #     pool = Pool(processors)
    #     results = pool.map(self.job, range(processors))
    #
    #     print(results)
    #     return results
    #
    ...
    #     process = Actor_Critic_Experience_Gatherer(self.object_copies[ix], exploration, self.queue)
    #     gradients, episode_rewards, episode_states, episode_actions = process.start()
...
#
class Actor_Critic_Experience_Gatherer(multiprocessing.Process):

    def __init__(self, agent, exploration, queue):
        super(Actor_Critic_Experience_Gatherer, self).__init__()
