...
class HRL(Base_Agent):
    ...
    def determine_rolling_score_to_beat_before_recalculating_grammar(self):
        ...
        target_rolling_score = self.rolling_score + (improvement_required * 0.1)
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        while self.episodes_conducted < self.num_episodes:
            ...
            macro_actions_seen = self.play_agent_until_progress_made(agent)


            print("macro_actions_seen", macro_actions_seen)
            latest_macro_actions_seen = macro_actions_seen[-2000:]

            action_id_to_actions = self.update_action_choices(latest_macro_actions_seen)



            print("New action_id_to_actions ", action_id_to_actions)
    ...
    def update_action_choices(self, latest_macro_actions_seen):
        ...
        print("global_action_id_to_primitive_action ", self.global_action_id_to_primitive_action)
        self.global_action_id_to_primitive_action.update(new_actions)
        print("global_action_id_to_primitive_action ", self.global_action_id_to_primitive_action)
    ...
    def generate_agent_with_updated_action_set(self):
        ...
        agent = DDQN_Wrapper(updated_config)
        return agent
...
class Environment_Wrapper(Wrapper):
    ...
    def step(self, macro_action):
        ...
        if len(actions) > 1:
            cumulative_reward = cumulative_reward * (1.0 + self.action_length_reward_bonus)
...
class DDQN_Wrapper(DDQN):
    ...
    def run_n_episodes(self, num_episodes, stop_when_progress_made=False):
        ...
        while self.episode_number < num_episodes and not self.progress_has_been_made():
            self.reset_game()
            self.step()
            self.save_and_print_result()
            if stop_when_progress_made:
                if self.progress_has_been_made(): break
    ...
    def progress_has_been_made(self):
        ...
        if len(self.rolling_results) == 0: return False
